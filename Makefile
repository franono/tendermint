...
LD_FLAGS = -X github.com/franono/tendermint/version.GitCommit=`git rev-parse --short=8 HEAD`
BUILD_FLAGS = -mod=readonly -ldflags "$(LD_FLAGS)"
HTTPS_GIT := https://github.com/franono/tendermint.git
...
draw_deps:
	...
	@goviz -i github.com/franono/tendermint/cmd/tendermint -d 3 | dot -Tpng -o dependency-graph.png
.PHONY: draw_deps
...
format:
	...
	find . -name '*.go' -type f -not -path "*.git*"  -not -name '*.pb.go' -not -name '*pb_test.go' | xargs goimports -w -local github.com/franono/tendermint
.PHONY: format
...
build-docker:
	...
	docker build --label=tendermint --tag="franono/tendermint" DOCKER
...
build_c-amazonlinux:
	...
	docker run --rm -it -v `pwd`:/tendermint franono/tendermint:build_c-amazonlinux
